...
def optimo(intentos):
    lamina=templadosimulado()
    optim=lamina
    sobra1=lamina[0].sobrante()
    T=1000 #Temperatura inicial
    Tmin=15 #Valor minimo de la temperatura
    t=0
    while(Tmin<T):
        lamina=templadosimulado()
        sobral1=lamina[0].sobrante()
        sobral2=lamina[1].sobrante()
        #Principio metropolis 
        p=math.exp(-(sobral2-sobral1)/T)
        r=np.random.uniform(low=0,high=1)
        if(sobral1<sobra1 or r < p):
            sobra1=sobral1
            optim=lamina
        t+=1
        T=1000/(1+t)
